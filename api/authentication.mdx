...
The response carries your `client_id`. Redirect URIs must be `https` (loopback `http://localhost` is allowed for native and local clients) and are matched character for character. Leave `token_endpoint_auth_method` at its default of `none` for a browser, mobile or desktop app; a server-side app may set it to `client_secret_basic` or `client_secret_post` to receive a `client_secret`, shown once.
