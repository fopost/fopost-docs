...
```swift
dependencies: [
    .package(url: "https://github.com/fopost/fopost-swift.git", from: "0.2.0")
]
```
...
`publish` returns when delivery is **queued**, not when the post is live. Poll `client.posts.deliveries(post.id)` or subscribe to [webhooks](/webhooks) for the outcome. `publish(_:dryRun: true)` validates the plan without sending anything, and `preflight` reports per-account blockers and advisory content signals.
...
| `client.inbox` | Comments, mentions and DMs as items, threads and conversations, the unread count, read marks, refresh, replies, state changes, hide, unhide, delete, likes, pins, reactions, comment edits, new conversations, the typing indicator, and reply approvals |
| `client.ads` | Ad connections and sources, boosting a published post, standalone ads, status, refresh, delete, audiences, targeting search, lead forms and leads |

On `client.ads`, `boost`, `create`, `setStatus` and `delete` spend money, so they need the `publish` scope as well as `ads`.
