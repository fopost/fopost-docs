...
Published on [Maven Central](https://central.sonatype.com/artifact/com.fopost/fopost-java). Source and issues: [github.com/fopost/fopost-java](https://github.com/fopost/fopost-java). MIT licensed.
...
`publish` returns once delivery is queued, not once it is live. Read `posts().deliveries()` or subscribe to [webhooks](/webhooks) for the result.
...
| `inbox()` | `list`, `threads`, `conversations`, `unreadCount`, `accounts`, `platforms`, `markThreadRead`, `markConversationRead`, `refresh`, `update`, `reply`, `hide`, `unhide`, `delete`, `like`, `unlike`, `pin`, `unpin`, `react`, `editComment`, `startConversation`, `setTyping`, `listApprovals`, `approveReply`, `rejectReply` |
