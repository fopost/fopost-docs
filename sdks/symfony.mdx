...
`$fopost->posts()`, `accounts()`, `workspaces()`, `labels()`, `ai()`, and the `request()` escape hatch are the full surface, documented on the [PHP SDK page](/sdks/php). The bundle adds nothing to it.
