...
Source and issues: [github.com/fopost/fopost-symfony](https://github.com/fopost/fopost-symfony). MIT licensed.
...
That mounts `POST /fopost/webhook`. Create a [webhook](/api/webhooks) pointing at it and copy the secret it shows once into `FOPOST_WEBHOOK_SECRET`. The controller verifies the HMAC-SHA256 signature over the raw request body, answers `401` when it does not match, and dispatches a Symfony event when it does.
