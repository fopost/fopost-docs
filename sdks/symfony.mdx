---
title: Symfony
description: The FoPost PHP client as a Symfony bundle, with autowiring, console commands, and webhook events.
icon: tech-php
---

`fopost/symfony-bundle` is a thin wrapper over the [PHP SDK](/sdks/php). Every request, retry, model, and error type lives there; the bundle wires it into Symfony: one configured client for autowiring, two console commands, and a webhook endpoint that turns deliveries into Symfony events.
...
Published on [Packagist](https://packagist.org/packages/fopost/symfony-bundle). Source and issues: [github.com/fopost/fopost-symfony](https://github.com/fopost/fopost-symfony). MIT licensed.
...
| `base_url` | the hosted API | API root. A host with no path gets the versioned path appended |
...
`$fopost->posts()`, `accounts()`, `workspaces()`, `labels()`, `ai()`, `inbox()`, `ads()`, and the `request()` escape hatch are the full surface, documented on the [PHP SDK page](/sdks/php). The bundle adds nothing to it.
...
That mounts `POST /fopost/webhook`. Create a [webhook](/webhooks) pointing at it and copy the secret it shows once into `FOPOST_WEBHOOK_SECRET`. The controller verifies the HMAC-SHA256 signature over the raw request body, answers `401` when it does not match, and dispatches a Symfony event when it does.
