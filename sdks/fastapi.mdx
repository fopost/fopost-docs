...
Published on [PyPI](https://pypi.org/project/fopost-fastapi/). Source and issues: [github.com/fopost/fopost-fastapi](https://github.com/fopost/fopost-fastapi). MIT licensed.
...
Everything you can call on the injected client (`posts`, `accounts`, `workspaces`, `labels`, `ai`, `inbox`, `ads`, and the `request()` escape hatch for endpoints the SDK does not wrap) is on the [Python SDK page](/sdks/python). This package adds no resources of its own and stores nothing.
