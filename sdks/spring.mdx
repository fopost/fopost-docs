...
Published on [Maven Central](https://central.sonatype.com/artifact/com.fopost/fopost-spring-boot-starter). Source and issues: [github.com/fopost/fopost-spring](https://github.com/fopost/fopost-spring). MIT licensed.
...
The full resource surface (`posts`, `accounts`, `workspaces`, `labels`, `webhooks`, `analytics`, `automations`, `media`, `ai`, `inbox`, `ads`), pagination, error types, and the `request` escape hatch are on the [Java SDK page](/sdks/java).
