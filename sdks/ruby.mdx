...
Comments, mentions, and DMs on connected accounts, through `client.inbox`: `list`, `threads`, `conversations`, `unread_count`, `accounts`, `platforms`, `mark_thread_read`, `refresh`, `update`, `reply`, `hide`, `unhide`, `delete`, `approvals`, `approve_reply`, and `reject_reply`. Needs the `inbox` scope.

```ruby
page = client.inbox.list(workspace_id: workspace.id, state: 'unread', sort: 'unanswered')
item = page.first

client.inbox.reply(item.id, text: 'Thanks, glad it helped!')
client.inbox.update(item.id, state: 'resolved')
```

## Ads

Boosts, standalone ads, audiences, and lead forms, through `client.ads`: `list`, `external`, `boostable`, `connections`, `sources`, `authorize_meta`, `delete_connection`, `boost`, `create`, `refresh`, `set_status`, `delete`, `audiences`, `create_audience`, `search_targeting`, `lead_forms`, `create_lead_form`, and `leads`. Needs the `ads` scope.
...
`boost`, `create`, `set_status`, and `delete` spend money, so they need the `publish` scope as well as `ads`.
