...
Published on [crates.io](https://crates.io/crates/fopost). Source and issues: [github.com/fopost/fopost-rust](https://github.com/fopost/fopost-rust). API docs on [docs.rs](https://docs.rs/fopost). MIT licensed.
...
Times are UTC. `publish` returns once delivery is queued, not once it is live. Read `posts().deliveries()` or subscribe to [webhooks](/webhooks) for the result.
