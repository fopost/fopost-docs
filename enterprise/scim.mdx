...
**Create.** A new member joins the workspace with the **member** role. If the person already has a FoPost account on that address, the existing account is reused rather than duplicated. Roles are managed in FoPost, not pushed from the directory, so promoting someone to admin is done in **Team**.

**Update.** A display name change is applied. An address change is applied only when the new address is on the same verified domain; anything else is refused with `400` so the mismatch shows up in your provisioning log instead of failing quietly.
...
| `404` | No such user in this workspace |
| `409` | A member with that `userName` already exists |
