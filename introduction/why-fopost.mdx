...
The [REST API](/api/overview) is the same one the dashboard runs on, included with every account — requests are free within the [rate limit](/api/rate-limits). Official [TypeScript](/sdks/typescript) and [Python](/sdks/python) clients, an [MCP server](/sdks/mcp) so an AI assistant can drive it, and [webhooks](/api/webhooks) for every publishing event.
