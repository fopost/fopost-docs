...
Every attempt, including each retry, is logged for 30 days with the status and body your endpoint answered, and any attempt can be sent again with the identical body. Both are in the dashboard under **Webhooks** and on the [webhooks API](/api/webhooks#the-delivery-log).
...
- The test is dispatched as a `post.published` event to this webhook only, whatever it subscribes to. A paused webhook receives nothing, and the endpoint still answers with success
...
Send the test and watch your logs. It remains the cheapest way to find out your handler is behind a firewall.
