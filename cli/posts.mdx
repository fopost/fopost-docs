...
A workspace is required: `--workspace` or the default saved by `auth login`. Files passed with `--media` are uploaded to the workspace's [media library](/guide/media-library) first, then attached.
