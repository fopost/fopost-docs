...
|:------|:----------|:-------------|
| Content | `Owlstack\Core\Content` | `Post`, `Media`, `MediaCollection`, `CanonicalLink`. Immutable, platform-agnostic |
| Config | `Owlstack\Core\Config` | `PlatformCredentials`, `OwlstackConfig`, `ConfigValidator` |
| Formatting | `Owlstack\Core\Formatting` | One formatter per network, plus `CharacterTruncator` and `HashtagExtractor` |
| Platforms | `Owlstack\Core\Platforms` | One class per network, plus `PlatformRegistry` |
| Publishing | `Owlstack\Core\Publishing` | `Publisher` and `PublishResult` |
| Auth | `Owlstack\Core\Auth` | `OAuthHandler`, `AccessToken`, and the token store contract |
| Events | `Owlstack\Core\Events` | `PostPublished`, `PostFailed`, and the dispatcher contract |
| Http | `Owlstack\Core\Http` | A cURL client with no dependencies |
| Support | `Owlstack\Core\Support` | `Arr`, `Str`, and a freezable `Clock` |
...
|:--------|:-----------|:--------|
| `owlstack/owlstack-core` | The engine. Everything above | MIT |
| `owlstack/owlstack-laravel` | Service provider, facade, config, queued events | MIT |
