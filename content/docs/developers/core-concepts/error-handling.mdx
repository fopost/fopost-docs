...
|:----------|:------|:--------|
| `FopostException` | Base for everything the library raises | |
...
Everything extends `RuntimeException`, so a single `catch (FopostException $e)` covers the library without swallowing unrelated failures.

## Catching directly

```php
use Fopost\Core\Exceptions\ContentTooLongException;
use Fopost\Core\Exceptions\MediaValidationException;
