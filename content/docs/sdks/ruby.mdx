...
Source and issues: [github.com/fopost/fopost-ruby](https://github.com/fopost/fopost-ruby). MIT licensed.
...
`accounts` takes account ids, the `Fopost::SocialAccount` objects themselves, or hashes with an `id`. `publish` returns once delivery is queued, not once it is live — read `client.posts.deliveries` or subscribe to [webhooks](/api/webhooks) for the result.
