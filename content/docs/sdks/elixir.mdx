...
To send something out now, create it and call `publish/3`. Nothing reaches a platform without that call or a schedule the user set, and publishing answers when delivery is **queued**, not when the post is live — poll `FoPost.Posts.deliveries/2` or subscribe a [webhook](/api/webhooks) for the outcome.
