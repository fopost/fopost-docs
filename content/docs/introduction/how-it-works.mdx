...
Whether the request comes from the composer, the [API](/api/overview), an [SDK](/sdks/overview), or the [agent](/guide/using-owlstack/owl-agent), the path is the same.

```
You  →  OwlStack  →  30 networks
```
...
Some networks have no supported way for a server to post for you, and driving a headless browser at one breaks their terms. For those, OwlStack stages the post and the [browser extension](/sdks/browser-extension) publishes it from your own logged-in session, in one click.
