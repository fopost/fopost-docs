...
```bash
curl -X POST https://api.owlstack.app/v1/automations \
...
The webhook entry point for an `api_webhook` automation. It authenticates with the trigger secret rather than your API key, so you can hand the URL to a system that has no OwlStack credentials.
...
<Cards>
  <Card title="Automations guide" href="/guide/using-owlstack/automations" />
