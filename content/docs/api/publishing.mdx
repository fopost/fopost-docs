...
Publishing in OwlStack is two steps: create a post, then publish it. They are separate because a post is a durable object with per-account deliveries, not a fire-and-forget request. Everything on this page needs the `posts` scope.
...
This answers `202 Accepted` and queues delivery. It does not wait for the platforms, so a `202` means accepted, not live. Poll [deliveries](#read-the-result) or subscribe to [webhooks](/api/webhooks) for the outcome.
