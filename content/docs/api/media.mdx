...
```bash
curl -X POST https://api.owlstack.app/v1/media/upload \
...
Each network sets its own limits on count, dimensions, aspect ratio, and duration. OwlStack resizes and re-encodes for the target where it can, and [preflight](/api/publishing#check-before-you-publish) tells you when it cannot.
...
<Cards>
  ...
  <Card title="Media Library guide" href="/guide/using-owlstack/media-library" />
</Cards>
